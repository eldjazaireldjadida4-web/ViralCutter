...
def resolve_durations(template_name, min_duration=None, max_duration=None,
    ...
    min_d = min_duration if min_duration is not None else tpl["min_duration"]
    max_d = max_duration if max_duration is not None else tpl["max_duration"]
    return (int(min_d), int(max_d), tpl)
